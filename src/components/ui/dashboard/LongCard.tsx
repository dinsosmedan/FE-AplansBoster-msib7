    ...
    return (
        <>
            <div className=" w-[300px] my-5">
                <Pie options={{ plugins: { tooltip: { callbacks: { label: (item) => `${item.label}: ${item.formattedValue} %`, } }, legend: { position: 'bottom', labels: { usePointStyle: true, pointStyle: 'circle' }, }, datalabels: { display: true, color: "white", formatter: (item) => `${item} ${isPercent ? '%' : ' '}` } } }} data={datalabel} />
            </div>
